...
if __name__ == "__main__" :
    ...
    mlfa_interp_generator = initialize_bs_call_interp(M_L=M_L, dim=dim, how="sparse")

    eng.workspace['M_L'] = M_L
    eng.workspace['dim'] = dim
    eng.workspace['how'] = 'sparse'
    eng.eval("mlfa_interp_generator = py.examples.bs.initialize_bs.initialize_bs_call_interp(M_L, dim, how);",nargout=0)
    ...
    mlfa_args = [Lmin,
                 ...
                 factor_N,
                 eng]
    filename_cvg = "mlfa_interp_test_cvg_bs_call_sparse_dim{}.txt".format(dim)
    filename_complexity = "mlfa_interp_test_complexity_bs_call_sparse_dim{}.txt".format(dim)
    filename_acc = "mlfa_interp_test_n_bs_call_sparse_dim{}.txt".format(dim)
